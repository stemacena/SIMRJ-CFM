...
// LISTA DOS 92 MUNICÍPIOS (Para Filtros)
...
// DICIONÁRIO COMPLETO DE COORDENADAS DOS 92 MUNICÍPIOS DO RJ (Solução de Fallback caso não tenha o endereço exato)
const cityCoordsRJ = {
...
// --- FUNÇÃO PARA ENDEREÇO EXATO (GEOLOCALIZAÇÃO OPENSTREETMAP) ---
// Essa função pesquisa a rua+cidade na API para trazer a latitude/longitude perfeita.
async function geocodeAddress(endereco, municipio) {
    ...
    return null; // Retorna null se não achar, para usar o dicionário de Fallback
}
...
function renderMuseums(data) {
    ...
    data.forEach(museum => {
        ...
        // LÓGICA DE COORDENADAS (Endereço Exato ou Fallback Perfeito da Cidade)
        let lat = museum.lat;
        let lng = museum.lng;
        ...
        if (!lat) {
            ...
            // Dispersão suave para pinos na mesma cidade não ficarem ocultos um sob o outro
...
// --- 5. FILTROS (RESTAURADOS EXATAMENTE COMO ANTES) ---
...
function openProfile(id) {
    ...
    // SELO MUSEÓLOGO DENTRO DO MODAL
    const badge = document.getElementById('modalBadgeMuseologo');
    if (m.museologo === "Sim") { badge.classList.remove('d-none'); } 
...
document.getElementById('csvFile').addEventListener('change', async function(e) {
    ...
    // Mostrar loading
    document.getElementById('upload-status').className = 'alert alert-info small p-2 d-block mt-2';
    document.getElementById('upload-status').innerText = 'Lendo arquivo e processando coordenadas. Aguarde...';

    Papa.parse(file, {
        header: true, skipEmptyLines: true,
        complete: async function(results) { await processCSVData(results.data); }
    });
});

async function processCSVData(rawData) {
    let cleanData = [];
    ...
    for (let index = 0; index < rawData.length; index++) {
        ...
        // Tenta pegar a coordenada exata na API (Delay opcional para não travar, mas em protótipos de até 50 vai liso)
...
// CADASTRO MANUAL COM CHAMADA DE COORDENADA
async function addManualMuseum(e) {
    ...
    // Tenta buscar a latitude/longitude exata
    let coords = await geocodeAddress(endereco, municipio);
    ...
    renderMuseums(museumsData);
    alert('Cadastrado com sucesso no mapa!');
